        ...
        <div class="cart-box">
          ...
          <ul class="cart-item-list">
            <li class="cart-item" v-for="(item,index) in list" :key="index">
              ...
              <div class="item-num">
                <div class="num-box">
                  <a href="javascript:;" @click="updataCatr(item,'-')">-</a>
...
export default {
  ...
  methods: {
    ...
    // 更新购物车数量和购物车单选状态
    updataCatr(item, type) {
      let quantity = item.quantity,
        selected = item.productSelected
      if (type == '-') {
        if (quantity == 1) {
          this.$message.warning('购买商品至少保留一件')
          return
        }
        --quantity
      } else if (type == '+') {
    ...
    rendrData(res) {
      ...
      this.checkdNum = this.list.filter(item => item.productSelected).length
    },
    // 购物车下单
    order() {
